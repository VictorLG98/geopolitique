...
> "El Océano Ártico representa la última gran frontera de disputas territoriales no resueltas de la era moderna, con el potencial de redefinir el equilibrio comercial global."
    ...
    comments: [
      {
        ...
        content: "Excelente análisis. La flota de rompehielos nucleares de Rusia es una ventaja asimétrica gigantesca. La OTAN va muy rezagada en infraestructura polar.",
      ...
      },
      {
        ...
        content: "El aspecto ecológico es preocupante. Abrir rutas comerciales a costa del ecosistema ártico traerá consecuencias globales irreversibles.",
        created_at: new Date(Date.now() - 3600000 * 5).toISOString()
      }
    ]
  },
  "geopolitica-semiconductores-taiwan": {
    id: 2,
    ...
    comments: [
      {
        id: 201,
        post_id: 2,
        author: "Ingeniero Tech",
        content: "La dependencia mundial de una sola empresa (TSMC) en un territorio tan disputado es el mayor riesgo geopolítico de la actualidad.",
        created_at: new Date(Date.now() - 3600000 * 12).toISOString()
      }
    ]
  },
  "litio-andino-triangulo-oro": {
    id: 3,
...
export default async function Page({ params }: { params: Promise<{ slug: string }> }) {
  ...
  try {
    ...
    console.warn(`Failed fetching details for ${slug}, utilizing resilient fallback data.`);
    post = fallback || null;
  }
  ...
  try {
    ...
    // If backend is offline, extract minimal list from fallback keys
    allPosts = Object.values(FALLBACK_DETAILS).map(p => ({
      id: p.id,
      slug: p.slug,
      title: p.title,
      summary: p.summary,
