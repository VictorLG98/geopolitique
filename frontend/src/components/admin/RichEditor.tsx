...
function SlashCommandMenu({ state, onSelect, onChangeIndex }: SlashMenuProps) {
  ...
  return (
    <div
      ref={menuRef}
      style={style}
      className="w-72 max-h-80 overflow-y-auto bg-white border border-[hsl(38,15%,85%)] rounded-xl shadow-xl py-1.5"
    >
      {state.items.length === 0 ? (
        <p className="px-4 py-2 text-xs text-[hsl(28,8%,55%)]">Sin resultados</p>
      ) : (
        (() => {
          let absIdx = 0;
                  ...
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-3 py-2 text-left transition-colors ${
                      idx === state.selectedIndex
                        ? 'bg-[hsl(38,24%,93%)]'
                        : 'hover:bg-[hsl(38,24%,96%)]'
                    }`}
                  >
                    <span className="w-8 h-8 flex items-center justify-center rounded-lg bg-[hsl(38,24%,93%)] text-[hsl(24,15%,20%)] text-xs font-bold shrink-0">
                      {item.icon}
                    </span>
                    <div className="min-w-0">
                      <p className="text-sm font-semibold text-[hsl(24,15%,15%)] leading-none mb-0.5">{item.title}</p>
                      <p className="text-xs text-[hsl(28,8%,50%)] truncate">{item.description}</p>
                    </div>
                  </button>
                );
              })}
...
export default function RichEditor({ value, onChange, placeholder, onUploadImage }: RichEditorProps) {
  ...
  if (!editor) {
    return (
      <div className="notion-editor-skeleton border border-[hsl(38,15%,85%)] rounded-2xl overflow-hidden">
        <div className="h-12 bg-[hsl(38,24%,93%)] animate-pulse" />
  ...
  return (
    <div className="notion-editor border border-[hsl(38,15%,85%)] rounded-2xl overflow-hidden bg-white">

      {/* ── Slim top bar ──────────────────────────────────────────── */}
      <div className="flex items-center justify-between px-4 py-2 border-b border-[hsl(38,15%,85%)] bg-[hsl(38,24%,97%)]">
        <p className="text-[10px] uppercase tracking-widest text-[hsl(28,8%,50%)] font-semibold select-none">
          Editor de contenido — escribe <kbd className="px-1 py-0.5 bg-white border border-[hsl(38,15%,82%)] rounded text-[10px] font-mono">/</kbd> para insertar bloques
        </p>
        <div className="flex items-center gap-2">
          <button type="button" title="Deshacer" onMouseDown={e => { e.preventDefault(); editor.chain().focus().undo().run(); }}
            className="p-1 text-[hsl(28,8%,50%)] hover:text-[hsl(24,15%,20%)] transition-colors rounded">
      ...
      <BubbleMenu editor={editor}>
        <div className="flex flex-wrap items-center gap-0.5 p-1.5 bg-[hsl(24,15%,12%)] rounded-xl shadow-2xl border border-white/10 max-w-sm">
          {/* Format */}
      ...
      {/* ── Footer stats ──────────────────────────────────────────── */}
      <div className="flex items-center justify-between px-4 py-2 border-t border-[hsl(38,15%,85%)] bg-[hsl(38,24%,97%)] text-[10px] text-[hsl(28,8%,50%)] font-mono select-none">
        <span>
          <kbd className="px-1 bg-white border border-[hsl(38,15%,82%)] rounded text-[9px]">/</kbd>
          {' '}bloques · <kbd className="px-1 bg-white border border-[hsl(38,15%,82%)] rounded text-[9px]">Ctrl+B</kbd> negrita ·{' '}
          <kbd className="px-1 bg-white border border-[hsl(38,15%,82%)] rounded text-[9px]">Ctrl+I</kbd> cursiva
        </span>
